...
void ghi_file_hoa_don_nhap(DS_HOA_DON& a)
{
	...
	ofstream fo;
	fo.open("thong_tin_hoa_don_nhap.txt", ios::out);
	fo << "";
	fo.close();
	...
	if (a != NULL)
	{
		if (tam == a->data.maKh)
		{
			for (int i = 0; i < size; i++)
			{
				...
				a->data.data_gh.hh.insert(a->data.data_gh.hh.begin(), b);//Thêm vào đầu
			}
		}
		else if (stoi(tam.substr(2, 4)) < stoi(a->data.maKh.substr(2, 4)))
		{
