...
void xuat_1_trang_hd_nhap_hang(int trang_ht, vector < vector <HoaDon> > ds_tam, string luaChonNgayThang, string size, string ngay_min, string ngay_max, string thang_nam, long long tong_tien)
{
    int tt = 0;
    for (int i = 0; i < 2; i++)
    {
    ...
    {
        gotoXY(68, 49);
        cout << "-" +luaChonNgayThang << ": " << " Tu: " << ngay_min << " den: " << ngay_max;
    }
    else
    {
        gotoXY(68, 49);
        cout << "-" + luaChonNgayThang << ": " << thang_nam;
    }
    gotoXY(65, 50);
    cout << "Tong tien nhap hang: " << tong_tien;
}
                    ...
                    if (_kbhit())
                    {
                        ...
                        else if (c == 13)
                        {
                            if (vi_tri_x == 65 && vi_tri_y == 7)
                            {
                                ...
                                xuat_1_trang_hd_nhap_hang(trang_ht, ds_tam, luaChonNgayThang, size, ngay_min, ngay_max, thang_nam, tong_tien);
                                xuat_hoa_don_nhap_hang(65, 7, ds_tam.at(trang_ht - 1).at(0), 12);
                                gotoXY(98, 5); cout << "BAN CO MUON SUA HOA DON NAY? AN ENTER DE SUA HOA DON!";
                                box(118 + 6, 45, 6, 2, 11, 1, 11, " " + to_string(trang_ht));
                                box(190, 50, 10, 2, 11, 3, 11, "  Esc");
...
// ====== Hóa đơn bán ====== 
void lay_ds_trang_hd_ban(Admin ad, int &trang, vector < vector <HoaDon> > &ds_tam, string size, string ngay_min, string ngay_max, string luaChonNgayThang, string thang_nam, long long& tong_tien)
{
    int vi_tri_x = 0;
    int vi_tri_y = 0;
    if (size == "Tat ca" && luaChonNgayThang == "Tat ca")
...
void xuat_1_trang_hd_ban_hang(int trang_ht, vector < vector <HoaDon> > ds_tam, string luaChonNgayThang, string size, string ngay_min, string ngay_max, string thang_nam, long long tong_tien)
{
    int tt = 0;
    box(118, 45, 5, 2, 11, 1, 11, " <<");
    box(118 + 6, 45, 6, 2, 11, 1, 11, " " + to_string(trang_ht));
...
void xu_li_1_trang_hd_ban_hang(Admin &ad, int trang_ht, vector < vector <HoaDon> >& ds_tam, bool &qua_trang, bool &lui_trang, bool &thoat,int &trang, string luaChonNgayThang, string size, string ngay_min, string ngay_max, string thang_nam, long long& tong_tien, bool& chon_lai_bo_loc)
{
    bool kt = true;
    bool cl = true;
    int vi_tri_x = 60;
                    ...
                    if (_kbhit())
                    {
                        ...
                        else if (c == 13)
                        {
                            if (vi_tri_x == 60 && vi_tri_y == 7)
                            {
                                menu_sua_tt_hd(ds_tam.at(trang_ht - 1).at(0).ma_hoa_don, ad);
                                ds_tam.clear();
                                lay_ds_trang_hd_ban(ad, trang, ds_tam, size, ngay_min, ngay_max, luaChonNgayThang, thang_nam, tong_tien);
                                xuat_1_trang_hd_ban_hang(trang_ht, ds_tam, luaChonNgayThang, size, ngay_min, ngay_max, thang_nam, tong_tien);
                                xuat_hoa_don_ban_hang(60, 7, ds_tam.at(trang_ht - 1).at(0), 12);
                                gotoXY(98, 5); cout << "BAN CO MUON SUA HOA DON NAY? AN ENTER DE SUA HOA DON!";
                                box(118 + 6, 45, 6, 2, 11, 1, 11, " " + to_string(trang_ht));
                                box(190, 50, 10, 2, 11, 3, 11, "  Esc");
...
void xuat_sua_ds_hoa_don_ban(Admin& ad)
{
    ...
    while (true)
    {
        ...
        if (trang == 1)
        {
            box(59, 2, 145, 50, 0, 0, 0, "");
            xuat_1_trang_hd_ban_hang(trang_ht, ds_tam, luaChonNgayThang, size, ngay_min, ngay_max, thang_nam, tong_tien);
            xu_li_1_trang_hd_ban_hang(ad, trang_ht, ds_tam, qua_trang, lui_trang, thoat, trang, luaChonNgayThang, size, ngay_min, ngay_max, thang_nam, tong_tien, chon_lai_bo_loc);
            if (thoat == true) // thoát
            {
                thoat = false;
                box(59, 5, 145, 53, 0, 0, 0, "");
