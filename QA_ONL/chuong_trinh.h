...
void sua_so_luong_ao_ton_kho(string ma_hang_hoa, DS_Hang_Hoa& a, int sl_ao_moi) {
...
void sua_so_luong_quan_ton_kho(string ma_hang_hoa, DS_Hang_Hoa& a, int sl_quan_moi) {
		...
		case 2: 
		{
			...
			while (kt_menu = true)
			{
				...
				case 1: 
				{
					...
					sua_so_luong_ao_ton_kho(ma_hh, a,sl);
					break;
				}
				case 2: 
				{
					...
					sua_so_luong_quan_ton_kho(ma_hh, a, sl);
					break;
				}
				case 0:
				{
...
void nhapHoaDon(HoaDon& a, Admin b)
{
	...
	if (loai_hd == "Ban")
	{
		do {
			...
			getline(cin, a.ma_kh);
			while_ma_kh(a.ma_kh);
		} while (check_ma_kh(a.ma_kh, b.quan_li_ds_kh.t) == false);

		do
		{
			cout << "Nhap ma hang hoa: "; getline(cin, a.ma_hang_hoa);
			while_full_ma_hh(a.ma_hang_hoa);
		} while (check_ma_hh(a.ma_hang_hoa, b.quan_li_ds_hang_hoa) == -1);
		...
		if (a.ma_hang_hoa[0] == 'A')
		{
			for (int i = 0; i < b.quan_li_ds_hang_hoa.ds_ao.size(); i++)
			{
				if (a.ma_hang_hoa == b.quan_li_ds_hang_hoa.ds_ao.at(i).ma_hh)
				{
					...
					a.thong_tin_hang.soLuongTonKho = b.quan_li_ds_hang_hoa.ds_ao.at(i).soLuongTonKho;
					break;
				}
			}
		}
		else
		{
			for (int i = 0; i < b.quan_li_ds_hang_hoa.ds_quan.size(); i++)
			{
				if (a.ma_hang_hoa == b.quan_li_ds_hang_hoa.ds_quan.at(i).ma_hh)
				{
					...
					a.thong_tin_hang.soLuongTonKho = b.quan_li_ds_hang_hoa.ds_quan.at(i).soLuongTonKho;
					break;
				}
			}
		}
		...
		do
		{
			cout << "So luong hang hien co: " << a.thong_tin_hang.soLuongTonKho << endl;
		...
		for (int i = 0; i < b.quan_li_ds_hang_hoa.ds_ao.size(); i++)
		{
			if (a.ma_hang_hoa == b.quan_li_ds_hang_hoa.ds_ao.at(i).ma_hh)
			{
				do
				{
					...
					while_so(tam);
					a.sl_mua = stoi(tam);
				} while (a.sl_mua == 0 || a.sl_mua > b.quan_li_ds_hang_hoa.ds_ao.at(i).soLuongTonKho);
				tien = b.quan_li_ds_hang_hoa.ds_ao.at(i).gia;
				a.thong_tin_hang.ten_hh = b.quan_li_ds_hang_hoa.ds_ao.at(i).ten_hh;
				a.thong_tin_hang.gia = b.quan_li_ds_hang_hoa.ds_ao.at(i).gia;
				a.thong_tin_hang.size = b.quan_li_ds_hang_hoa.ds_ao.at(i).size;
				b.quan_li_ds_hang_hoa.ds_ao.at(i).soLuongTonKho = b.quan_li_ds_hang_hoa.ds_ao.at(i).soLuongTonKho - a.sl_mua;
				break;
			}
		}
	}
...
void thong_ke_hoa_don(Admin ad)
{
	...
	do
	{
		cout << "Nhap size muon loc (All/Size):";
		getline(cin, size);
		size[0] = toupper(size[0]);
	} while (check_size_ao(size) == false && check_size_quan(size) == false && size != "All");
			...
			case 5:
			{
				...
				if (tam1 == tam2)
				{
					...
					else
					{
						...
						{
							int tam3 = tinh_giay_cua_ngay_nhap_tu_nam_1900(k->data.ngay_lap_hd);

							if (tam1 == tam3 && k->data.thong_tin_hang.size == size)
							{
								xuatHoaDonBanHang(k->data);
								tong_tien += k->data.tong_tien;
							}
						}
						cout << "Tong so tien ban hang tu " << ngay_1 << " den ngay " << ngay_2 << "	:" << tong_tien;
						system("pause");
				...
				else if (tam1 > tam2)
				{
						...
						else
						{
							...
							for (node_hoa_don* k = ad.quan_li_ds_hoa_don_xuat.pHead; k != NULL; k = k->pNext)
							{
								...
								if (tam2 <= tam3 && tam3 <= tam1 && k->data.thong_tin_hang.size == size)
								{
									xuatHoaDonBanHang(k->data);
									tong_tien += k->data.tong_tien;
								}
							}
							cout << "Tong so tien ban hang tu " << ngay_2 << " den ngay " << ngay_1 << ":" << tong_tien;
				...
				else if (tam2 > tam1)
				{
						...
						else
						{
							...
							for (node_hoa_don* k = ad.quan_li_ds_hoa_don_xuat.pHead; k != NULL; k = k->pNext)
							{
								...
								if (tam1 <= tam3 && tam3 <= tam2 && k->data.thong_tin_hang.size == size)
								{
									xuatHoaDonBanHang(k->data);
									tong_tien += k->data.tong_tien;
								}
...
void duyet_cay_sua_thong_tin_so_luong_mua(TREE& t, string ma_hd_sua, string ma_kh,int sl_mua)
{
	if (t != NULL)
	{
		if (ma_kh == t->data.maKh)
...
void sua_thong_tin_hoa_don(Admin& ad)
{
	...
	if (b[0] == 'N')
	{
		while (check == true)
		{
			...
			cout << "\n3.Sua so luong mua cua hoa don";
			cout << "\n4.Xoa hoa don";
			cout << "\n0.Thoat";
			cout << "\nNhap lua chon: ";
	...
	else
	{
		while (check == true)
		{
			...
			cout << "5.Sua so luong cua hoa don" << endl;
			cout << "6.Xoa hoa don";
			cout << "\n0.Thoat";
			...
			case 1:
			{
				...
				for (node_hoa_don* k = ad.quan_li_ds_hoa_don_xuat.pHead; k != NULL; k = k->pNext)
				{
					if (k->data.ma_hoa_don == b)
					{
						...
						{
							k->data.tong_tien = (tien * k->data.sl_mua) * (0.97);
							k->data.giam_gia_theo_hang = 0.03;
						}
						else if (k->data.ma_kh.substr(0, 2) == "VA")
						{
						...
						{
							k->data.tong_tien = (tien * k->data.sl_mua) * (0.9);
			...
			case 5:
			{
				...
				for (node_hoa_don* k = ad.quan_li_ds_hoa_don_xuat.pHead; k != NULL; k = k->pNext)
				{
					if (k->data.ma_hoa_don == b)
					{
						if (k->data.sl_mua > sl_mua)
						{
							sl = sl + (k->data.sl_mua - sl_mua);
						}
						else if (k->data.sl_mua < sl_mua)
						{
							sl = sl - (sl_mua - k->data.sl_mua);
						}
						duyet_cay_sua_thong_tin_so_luong_mua(ad.quan_li_ds_kh.t, b, k->data.ma_kh, sl_mua);
						k->data.sl_mua = sl_mua;
						break;
					}
				}
...
void nhap_MK(char a[100])
{
	int i = 0;
	char c;
	while (true) {
		c = _getch();  // Sử dụng hàm _getch() để lấy ký tự từ bàn phím mà không hiển thị lên màn hình

		if (c == 13)  // 13 là mã ASCII của phím Enter
			break;
		else if (c == 13){  // 8 là mã ASCII của phím Backspace
			if (i > 0) {
				cout << "\b \b";  // Xóa ký tự cuối cùng và di chuyển con trỏ về trái
				i--;
			}
			}
		else {
			a[i] = c;
				cout << "*";  // In dấu sao thay vì ký tự thật
				i++;
		}
	}
	a[i] = '\0';
...
void dang_ky_tai_khoan(DS_KH& dskh, KhachHang& a)
{
	...
	getline(cin, a.sdt);
	while_Sdt(a.sdt);
	//while_kt_trung_sdt(a.sdt ,dskh.t);
	...
	getline(cin, a.email);
	while_email(a.email);
	//while_kt_trung_email(a.email, dskh.t);

	cout << "Ten tai khoan: ";
	getline(cin, a.ten_dang_nhap);
	while_tk(a.ten_dang_nhap);
	//while_kt_trung_ten_dn(a.ten_dang_nhap, dskh.t);
	...
	while (xac_nhan_mk != a.mat_khau)
	{
		...
		cout << "Xac nhan mat khau: "; nhap_MK(xac_nhan_mk);
		cout << "\n";
	}
	a.maKh = "SA" + tao_ma_khach_hang(dskh);
	a.ngay_dk = thoiGianThuc(a.ngay_dk);
...
void xuat_thong_tin_khach_hang_theo_ma(TREE t, string makh)
{
	if (t != NULL)
	{
		if (makh == t->data.maKh)
		...
		{
			xuat_thong_tin_khach_hang_theo_ma(t->pLeft, makh);
		}
		else
		{
			xuat_thong_tin_khach_hang_theo_ma(t->pRight, makh);
		}
	}
}
...
void xuat_don_mua(TREE t, string ma_kh)
{
	if (t != NULL)
	{
		...
		{
			xuat_don_mua(t->pLeft, ma_kh);
		}
		else
		{
			xuat_don_mua(t->pRight, ma_kh);
		}
	}
}
	...
	while (kt_menu==true) {
		...
		case 2:
		{
			...
			if (suaDiaChi(maKh, t, diaChiMoi)==true) {
				cout << "Sua thanh conng dia chi moi!" << endl;
			}
			else {
				cout << "Sua that bai!" << endl;
			}
		...
		case 3:
		{
			...
			if (suaSdt(maKh, t, sdtMoi) == true) {
				cout << "Sua thanh conng so dien thoai moi!" << endl;
			}
			else {
				cout << "Sua that bai!" << endl;
			}
		...
		case 4:
		{
			...
			if (suaEmail(maKh, t, emailMoi) == true) {
				cout << "Sua thanh conng email moi!" << endl;
			}
			else {
				cout << "Sua that bai!" << endl;
			}
		...
		case 5:
		{
			...
			if (doiMatKhau(maKh, t, matkhauMoi) == true) {
				cout << "Sua thanh conng mat khau moi!" << endl;
			}
			else {
				cout << "Sua that bai!" << endl;
			}
...
void suaThongTinKhTheoMa(TREE& t, string maKh) 
{
	bool kt_menu = true;
	int chon;
...
void duyet_cay_xem_gio_hang(TREE t, string makh)
{
	...
	if (t != NULL)
	{
		if (makh == t->data.maKh)
		{
			for (int i = 0; i < t->data.data_gh.hh.size(); i++)
			{
				xuat_tt_hh_ngan_gon(t->data.data_gh.hh[i]);
			}
			return;
		}
		else if (stoi(makh.substr(2, 4)) < stoi(t->data.maKh.substr(2, 4)))
		{
			duyet_cay_xem_gio_hang(t->pLeft, makh);
		}
		else
		{
			duyet_cay_xem_gio_hang(t->pRight, makh);
		}
	}
}

void duyet_cay_xoa_sp_gio_hang(TREE& t, string makh)
{
	...
	if (t != NULL)
	{
		if (makh == t->data.maKh)
		{
			...
			while_full_ma_hh(ma_sp_xoa);
			for (int i = 0; i < t->data.data_gh.hh.size(); i++)
			{
				if (t->data.data_gh.hh[i].ma_hh == ma_sp_xoa)
				{
					t->data.data_gh.hh.erase(t->data.data_gh.hh.begin() + i);
					cout << "San pham da duoc xoa khoi danh sach!" << endl;
					system("pause");
					return;
				}
			}
			cout << "San pham khong ton tai trong gio hang!" << endl;
			system("pause");
			return;
		}
		else if (stoi(makh.substr(2, 4)) < stoi(t->data.maKh.substr(2, 4)))
		{
			duyet_cay_xoa_sp_gio_hang(t->pLeft, makh);
		}
		else
		{
			duyet_cay_xoa_sp_gio_hang(t->pRight, makh);
		}
	}
}

void duyet_cay_them_sp_vao_gio_hang(TREE &t, string makh, HangHoa a)
{
	...
	if (t != NULL)
	{
		if (makh == t->data.maKh)
		{
			t->data.data_gh.hh.insert(t->data.data_gh.hh.begin(), a);
			return;
		}
		else if (stoi(makh.substr(2, 4)) < stoi(t->data.maKh.substr(2, 4)))
		{
			duyet_cay_them_sp_vao_gio_hang(t->pLeft, makh,a);
		}
		else
		{
			duyet_cay_them_sp_vao_gio_hang(t->pRight, makh,a);
		}
	}
}

void them_sp_vao_gio_hang(DS_Hang_Hoa dshh, TREE& t, string makh)
{
	bool kt_menu = true;
	int lua_chon;
	string k;
	...
	{
		cout << "1.Them Ao." << endl;
		cout << "2.Them Quan." << endl;
		cout << "0.Thoat." << endl;
		cout << "\nNhap lua chon: ";
		getline(cin, k);
		lua_chon = stoi(k);
		while (lua_chon > 2 || check_So(k) == false)
		{
			cout << "\nNhap khong hop le!";
			cout << "\nNhap lua chon: ";
			getline(cin, k);
		...
		case 1:
		{
			string ma_ao_mua;
			cout << "Nhap ma ao mua:";
			getline(cin, ma_ao_mua);
			while_full_ma_hh(ma_ao_mua);
			while (check_ma_hh(ma_ao_mua, dshh) == -1 || ma_ao_mua[0] == 'Q')
			{
				cout << "Khong tim thay, nhap lai:"; getline(cin, ma_ao_mua);
				while_full_ma_hh(ma_ao_mua);
			}
			for (int i = 0; i < dshh.ds_ao.size(); i++)
			{
				...
				{
					duyet_cay_them_sp_vao_gio_hang(t, makh, dshh.ds_ao[i]);
		...
		case 2:
		{
			...
			while_full_ma_hh(ma_quan_mua);
			while (check_ma_hh(ma_quan_mua, dshh) == -1 || ma_quan_mua[0] == 'A')
			{
				cout << "Nhap ma hang hoa khong phu hop, nhap lai: "; getline(cin, ma_quan_mua);
				while_full_ma_hh(ma_quan_mua);
			}
			for (int i = 0; i < dshh.ds_quan.size(); i++)
			{
				...
				{
					duyet_cay_them_sp_vao_gio_hang(t, makh, dshh.ds_quan[i]);
					cout << "San pham da duoc them vao gio hang!" << endl;
					system("pause");
					return;
				}
	...
	if (t != NULL)
	{
		if (makh == t->data.maKh)
		{
			for (node_hoa_don* k = t->data.don_mua_hang.pHead; k != NULL; k = k->pNext)
			{
				...
				{
					if (k->data.trang_thai == "Chua nhan hang")
					{
						string luachon;
						...
						if (luachon != "1")
						{
							thoiGianThuc(k->data.ngay_lap_hd);
...
bool duyet_cay_xac_nhan_hang(TREE& t, string makh, string mahd)
{
	if (t != NULL)
	{
		if (makh == t->data.maKh)
		{
			for (node_hoa_don* k = t->data.don_mua_hang.pHead; k != NULL; k = k->pNext)
			{
				if (mahd == k->data.ma_hoa_don)
				{
					...
					{
						thoiGianThuc(k->data.ngay_lap_hd);
						k->data.trang_thai = "Da Nhan Hang";
						cout << "\nDa xac nhan nhan hang!";
						return true;
					}
					else
					{
						cout << "\nDon hang khong the xac nhan da nhan hang!";
						return false;
					}
				}
			}
		...
		{
			duyet_cay_xac_nhan_hang(t->pLeft, makh,mahd);
		}
		else
		{
			duyet_cay_xac_nhan_hang(t->pRight, makh,mahd);
		}
	}
	else
	{
...
void tra_hang(Admin &ad, string makh,string mahd)
{
	if (duyet_cay_tra_hang(ad.quan_li_ds_kh.t, makh, mahd) == true) // đã xác nhận trả hàng
	{
		for (node_hoa_don* k = ad.quan_li_ds_hoa_don_xuat.pHead; k != NULL; k = k->pNext)
		{
			if (mahd == k->data.ma_hoa_don)
			{
				...
				xoa_hd_theo_ma(ad.quan_li_ds_hoa_don_xuat, mahd);
...
void xac_nhan_don_hang(Admin& ad, string makh)
{
	string mahd;
	int luaChon;
	string k;
	...
	cout << "\nNhap ma hoa don muon xac nhan: ";
	getline(cin, mahd);
	mahd[0] = toupper(mahd[0]);
	while (regex_match(mahd, ktra_dinh_dang) == false || check_trung_ma_hd(mahd, ad.quan_li_ds_kh.t->data.don_mua_hang) == -1)
	{
	...
	while (check == true)
	{
		...
		case 1:
		{
			tra_hang(ad, makh, mahd);
			break;
		}
		case 2:
		{
			duyet_cay_xac_nhan_hang(ad.quan_li_ds_kh.t, makh, mahd);
			break;
		}
		case 0:
		{
...
{
	if (ad.quan_li_don_hang_cho.don_cho.size() == 0)
	{
		cout << "\nHien khong co don cho!\n";
		return;
	}
	int luachon;
	string tam;
...
void xuat_ds_don_hang_tra(Admin ad)
{
	...
	{
		cout << "Ma khach hang: " << k->data.ma_kh << endl;
		cout << "Ma hoa don: " << k->data.ma_hoa_don << endl;
		cout << "\tThong tin hang hoa: " << endl;
		cout << "Ma hang hoa: " << k->data.ma_hang_hoa << endl;
		cout << "Ten hang hoa: " << k->data.thong_tin_hang.ten_hh << endl;
		cout << "Gia: " << k->data.thong_tin_hang.gia << endl;
		cout << "Size: " << k->data.thong_tin_hang.size << endl;
		cout << "So luong mua: " << k->data.sl_mua << endl;
		cout << "Ngay tra hang: " << k->data.ngay_lap_hd << endl;
		cout << "Gia duoc giam: " << k->data.giam_gia_theo_hang << endl;
		cout << "Tong tien: " << k->data.tong_tien << endl;
		cout << "Trang thai: " << k->data.trang_thai << endl;
		cout << "\t\t========================\n";
	}
}
...
bool compareLastNames(string& name1, string& name2) {
	string lastName1 = name1.substr(name1.find_last_of(' ') + 1);
	string lastName2 = name2.substr(name2.find_last_of(' ') + 1);

	return lastName1 < lastName2;
}

void duyet_cay_them_khach_hang_vao_mang(TREE t, vector<KhachHang>& tam)
{
	if (t != NULL)
	{
		tam.push_back(t->data);
		duyet_cay_them_khach_hang_vao_mang(t->pLeft, tam);
		duyet_cay_them_khach_hang_vao_mang(t->pRight, tam);
	}
}
